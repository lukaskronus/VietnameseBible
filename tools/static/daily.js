...
fetch(base() + "/data/chapters.json")
  ...
  .then(function (chs) {
    ...
    return fetch(base() + "/data/ch/" + id + ".json").then(function (r) {
      return r.json();
    });
  })
  .then(function (d) {
    ...
    for (var i = 0; i < d.blocks.length; i++) {
      ...
      if (b.t === "h") {
        ...
        h += '<p class="reading vi"><sup>' + b.n + "</sup>" + esc(vi) + "</p>";
        if (en) {
          h += '<p class="reading en" lang="en"><sup>' + b.n + "</sup>" + esc(en) + "</p>";
        }
        h += "</div>";
      }
    }
    document.getElementById("daily-text").innerHTML = h;
  })
  .catch(function () {
